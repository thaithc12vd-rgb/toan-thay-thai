...
<style>
    ...
    .certificate {{
        ...
        font-family: 'Times New Roman', serif; color: #333; margin: 20px 0;
    }}
...
if role == "teacher":
    ...
    with col_r:
        if pwd == "thai2026":
            ...
            # Cập nhật dữ liệu khi chọn đề cũ
            ...
            # HIỆN LINK KHI CÓ MÃ ĐỀ (Kể cả khi chọn đề cũ)
            if m_de:
                base_url = "https://toan-thay-thai-spgcbe5cuemztnk5wuadum.streamlit.app/"
                share_link = f"{base_url}?de={m_de}"
                st.success(f"🔗 Link gửi học sinh: {share_link}")
                st.code(share_link)
    ...
    if ma_de_url in library:
        ...
        if st.session_state.is_accepted and not st.session_state.is_submitted:
            ans_dict = {}
            valid_qs = [x for x in library[ma_de_url] if x["q"]]
            for idx, item in enumerate(valid_qs, 1):
            ...
            if st.button("📝 NỘP BÀI", use_container_width=True, type="primary"):
                dung = sum(1 for idx, it in enumerate(valid_qs, 1) if str(ans_dict.get(f"Câu {idx}", "")).strip().lower() == str(it["a"]).strip().lower())
                diem = int((dung / len(valid_qs)) * 10)
        ...
        if st.session_state.is_submitted:
            ...
            if st.session_state.current_rank <= 10:
                ...
                st.download_button("📥 TẢI GIẤY KHEN", data=gk_html, file_name=f"GiayKhen_{st.session_state.student_name}.html", mime="text/html")
