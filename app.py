...
if role == "teacher":
    ...
    with col_l:
        ...
        if pwd == "thai2026":
            ...
            # THÊM NÚT LIVE VÀ NÚT ẨN BÊN TRÁI
            st.markdown('<div class="live-btn">', unsafe_allow_html=True)
            if st.button("🔴 NHẤN VÀO ĐÂY ĐỂ XEM LIVE"): st.session_state.view_live = True
            st.markdown('</div>', unsafe_allow_html=True)
            st.markdown('<div class="hide-btn">', unsafe_allow_html=True)
            if st.button("⚪ ẨN LIVE"): st.session_state.view_live = False
            st.markdown('</div>', unsafe_allow_html=True)
            
            # HIỂN THỊ SỐ LƯỢNG HỌC SINH LÀM BÀI CỘNG DỒN
            m_de_cnt = st.text_input("Nhập Mã đề để đếm tổng:", key="cnt_de").strip()
            if m_de_cnt:
                total_em = sum(1 for k in profiles.keys() if m_de_cnt in k)
                st.info(f"Tổng số các em làm bài: {total_em}")
    ...
    with col_r:
        if pwd == "thai2026":
            if st.session_state.view_live:
                st.markdown('<div class="card">', unsafe_allow_html=True)
                m_de_live = st.text_input("Nhập Mã đề để hiện Live:", key="live_de").strip()
                if m_de_live:
                    st.markdown(f"### 🏆 TOP 100 ĐANG LIVE: {m_de_live}")
                    dt_live = doc_file(FILE_RES).get(m_de_live, [])
                    if dt_live:
                        df_l = pd.DataFrame(dt_live).sort_values(by=['score', 'time'], ascending=[False, True]).reset_index(drop=True)
                        df_l.index += 1
                        st.table(df_l.head(100).rename(columns={'student':'Học sinh','score':'Điểm','time':'Giờ nộp'}))
                st.markdown('</div>', unsafe_allow_html=True)
