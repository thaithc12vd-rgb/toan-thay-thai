...
<style>
    ...
    .line-separator {{
        width: 3cm; height: 2px; background-color: #004F98; margin: 5px auto;
    }}
...
<div class="sticky-header">
    ...
    {f'<div class="student-display">{st.session_state.student_name}</div><div class="line-separator"></div>' if st.session_state.is_accepted and st.session_state.student_name else ""}
</div>
...
if role == "teacher":
    ...
    with col_l:
        ...
        if pwd == "thai2026":
            ...
            if st.button("📂 QUẢN LÝ ĐỀ TRONG KHO"): st.session_state.show_storage = True
        st.markdown('</div>', unsafe_allow_html=True)

    with col_r:
        if pwd == "thai2026":
            if st.session_state.get('show_storage'):
                st.markdown('<div class="card"><h3>📦 KHO ĐỀ</h3>', unsafe_allow_html=True)
                for d_key in list(library.keys()):
                    c1, c2 = st.columns([4, 1])
                    c1.write(f"🔹 **Mã đề:** {d_key}")
    ...
    if ma_de_url in library:
        if not st.session_state.is_accepted:
            ...
            name_in = st.text_input("Nhập tên em:", key="name_input").strip()
