...
<style>
    ...
    .counter-box {{ background-color: #1A2238; color: #FFD700; padding: 10px; border-radius: 10px; text-align: center; font-weight: bold; margin-top: 10px; border: 1px solid #FFD700; }}
</style>
...
library = load_json(FILE_DB)
profiles = load_json(FILE_PROF)
results_all = load_json(FILE_RES)

# Khởi tạo session an toàn
cac_bien = [('is_accepted', False), ('is_submitted', False), ('cau_hoi_hien_tai', []), ('student_name', ""), 
            ('current_rank', 0), ('final_score', 0), ('view_live', False), ('ver_key', 0)]
for k, v in cac_bien:
...
if role == "teacher":
    ...
    with col_l:
        ...
        if pwd == "thai2026":
            ...
            # --- CỤM NÚT LIVE BÊN TRÁI ---
            st.markdown('<div class="live-btn">', unsafe_allow_html=True)
            if st.button("🔴 HIỆN BẢNG LIVE"): st.session_state.view_live = True
            st.markdown('</div><div class="hide-btn">', unsafe_allow_html=True)
            if st.button("⚪ ẨN BẢNG LIVE"): st.session_state.view_live = False
            st.markdown('</div>', unsafe_allow_html=True)

            # --- BỘ ĐẾM TỔNG CỘNG DỒN ---
            m_de_check = st.text_input("Mã đề để đếm tổng:", key="m_de_cnt").strip()
            if m_de_check:
                total_attempts = sum(p.get("attempts", 0) for k, p in profiles.items() if m_de_check in k)
                st.markdown(f'<div class="counter-box">TỔNG LƯỢT LÀM BÀI: {total_attempts}</div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with col_r:
        if pwd == "thai2026":
            if st.session_state.view_live:
                st.markdown('<div class="card">', unsafe_allow_html=True)
                m_de_live = st.text_input("Nhập Mã đề xem Live:", key="live_de").strip()
                if m_de_live:
                    st.markdown(f"### 🏆 BẢNG VÀNG LIVE: {m_de_live}")
                    data_live = results_all.get(m_de_live, [])
                    if data_live:
                        df_live = pd.DataFrame(data_live).sort_values(by=['score', 'time'], ascending=[False, True]).reset_index(drop=True)
                        df_live.index += 1
                        st.table(df_live.head(100).rename(columns={'student':'Học sinh','score':'Điểm','time':'Giờ nộp'}))
                st.markdown('</div>', unsafe_allow_html=True)
            else:
                st.info("Bảng Live đang ẩn. Thầy có thể quản lý đề tại đây.")
                # Logic soạn thảo đề (giữ nguyên bản gốc của thầy)
    ...
    if ma_de_url in library:
        ...
        if not st.session_state.is_accepted:
            ...
            with c2:
                name_in = st.text_input("", key="st_name", label_visibility="collapsed", autocomplete="off").strip()
                if st.button("ĐỒNG Ý", use_container_width=True, type="primary"):
                    if name_in:
                        ...
                        if prof["attempts"] >= 20: st.error("Đã hết lượt làm bài!")
                        else:
                            # Cộng dồn lượt làm bài ngay khi nhấn Đồng ý
                            prof["attempts"] += 1
                            profiles[sk] = prof; save_json(FILE_PROF, profiles)
        ...
        if st.session_state.is_accepted and not st.session_state.is_submitted:
            ...
            for idx, item in enumerate(st.session_state.cau_hoi_hien_tai, 1):
                ...
                ans_dict[f"Câu {idx}"] = st.text_input(f"Đáp án {idx}", key=f"ans_{idx}", label_visibility="collapsed", autocomplete="off")
            
            if st.button("📝 NỘP BÀI", use_container_width=True, type="primary"):
                # CHẤM ĐIỂM CHÍNH XÁC THEO ĐỀ MỚI
                ...
                # Cập nhật hạng và Profile
                df_t = pd.DataFrame(results_all[ma_de_url]).sort_values(by=['score', 'time'], ascending=[False, True]).reset_index(drop=True)
                ...
                prof = profiles.get(sk, {"attempts": 1, "top10_count": 0})
        ...
        if st.session_state.is_submitted:
            ...
            st.markdown(f"### 📊 TỔNG SỐ BẠN ĐÃ THAM GIA: {len(data_live)}")
            if data_live:
