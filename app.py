...
# ==========================================
# CỔNG QUẢN TRỊ (QUY TRÌNH NGHIÊM NGẶT)
# ==========================================
if role == "teacher":
    ...
    with col_l:
        ...
        # KHÔI PHỤC Ô NHẬP API KEY
        st.markdown('<span class="small-inline-title" style="margin-top:15px;">🤖 CẤU HÌNH AI</span>', unsafe_allow_html=True)
        api = st.text_input("API Key", value=config.get("api_key", ""), type="password", placeholder="Nhập Gemini API...", key="admin_api", label_visibility="collapsed")
        if st.button("LƯU API", use_container_width=True):
    ...
    with col_r:
        if pwd == "thai2026":
            ...
            # KHÔI PHỤC BƯỚC 2: DÒNG LINK & NÚT COPY
            if m_de:
            ...
            st.warning("Vui lòng nhập đúng mật mã để quản trị.")

# ==========================================
# CỔNG HỌC SINH (GIỮ NGUYÊN)
# ==========================================
else:
    # (Giữ nguyên toàn bộ logic làm bài và xếp hạng của học sinh)
