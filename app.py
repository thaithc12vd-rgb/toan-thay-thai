...
<style>
    ...
    .live-btn button {{ background-color: #d32f2f !important; color: white !important; font-weight: bold !important; border-radius: 10px; border: 2px solid white !important; }}
</style>
...
# --- 2. BỘ MÁY BIẾN ĐỔI ĐỀ BÀI (TỰ CẬP NHẬT ĐÁP ÁN ĐỂ CHẤM ĐIỂM) ---
...
def bien_doi_cau_hoi(q_text, a_text):
    ...
    # Logic tính toán lại đáp án để chấm điểm không bị sai
    da_moi = a_text
    try:
...
def quet_don_48h(results):
    ...
    for de, ds in results.items():
        ...
        for r in ds:
            try:
                tg = datetime.strptime(r.get('full_time', '2000-01-01 00:00:00'), "%Y-%m-%d %H:%M:%S")
...
results_all = doc_file(FILE_RES) # Load toàn bộ để tính tổng vĩnh viễn
...
if role == "teacher":
    ...
    with col_l:
        ...
        if pwd == "thai2026":
            ...
            # NÚT LIVE PHÍA DƯỚI UPLOAD FILE (BÊN TRÁI)
    ...
    with col_r:
        if pwd == "thai2026":
            if st.session_state.view_live:
                st.markdown('<div class="card">', unsafe_allow_html=True)
                m_de_live = st.text_input("Nhập Mã đề để xem Live:", key="live_de").strip()
                if m_de_live:
                    st.markdown(f"### 🏆 TOP 100 ĐANG LIVE: {m_de_live}")
                    data_live = doc_file(FILE_RES).get(m_de_live, [])
                    if data_live:
            ...
            m_de = st.text_input("👉 Mã đề:", value=de_chon if de_chon != "-- Tạo mới --" else "").strip()
            if m_de: st.code(f"https://toan-thay-thai-spgcbe5cuemztnk5wuadum.streamlit.app/?de={m_de}")
            if st.button("🚀 LƯU ĐỀ"):
                if m_de:
                    n = len(st.session_state.data_step3) if st.session_state.data_step3 else 5
                    library[m_de] = [{"q": st.session_state.get(f"q_{st.session_state.ver_key}_{i}", ""), "a": st.session_state.get(f"a_{st.session_state.ver_key}_{i}", "")} for i in range(1, n + 1)]
                    ghi_file(FILE_DB, library); st.success("Đã lưu!"); st.rerun()
            for i in range(1, (len(st.session_state.data_step3) or 5) + 1):
    ...
    if ma_de_url in library:
            ...
            with c2:
                ...
                if st.button("ĐỒNG Ý", use_container_width=True, type="primary"):
                    if name_in:
                        ...
                        if doc_file(FILE_PROF).get(sk, {}).get("attempts", 0) >= 20: st.error("Làm quá 20 lần!")
                        else:
        ...
        if st.session_state.is_accepted and not st.session_state.is_submitted:
            ...
            if st.button("📝 NỘP BÀI", use_container_width=True, type="primary"):
                # CHẤM ĐIỂM CHUẨN XÁC THEO ĐỀ ĐÃ ĐỔI SỐ
                ...
                dt = pd.DataFrame(r_all[ma_de_url]).sort_values(by=['score', 'time'], ascending=[False, True]).reset_index(drop=True)
        ...
        if st.session_state.is_submitted:
            ...
            # BẢNG VÀNG LIVE HỌC SINH
            st.markdown('<div class="card">', unsafe_allow_html=True)
            all_data = doc_file(FILE_RES).get(ma_de_url, [])
            st.markdown(f"### 📊 TỔNG SỐ BẠN ĐÃ LÀM BÀI NÀY: {len(all_data)}")
            if all_data:
                # Lọc 48h cho bảng hiển thị nhưng vẫn giữ data vĩnh viễn trong file
                ds_hien_thi = quet_don_48h({ma_de_url: all_data})[ma_de_url]
                df = pd.DataFrame(ds_hien_thi).sort_values(by=['score', 'time'], ascending=[False, True]).reset_index(drop=True)
